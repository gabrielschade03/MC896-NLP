...
<div class="app" id="app">
  ...
  <main class="shell">
    ...
    <nav class="tabs" aria-label="Visualizações">
      ...
      <button class="tab" data-panel="timeline-panel" type="button">Linha do tempo</button>
    ...
    <section class="panel" id="timeline-panel">
      <div class="timeline-wrap"><div class="timeline-tools"><div><div class="timeline-title">Sequência de eventos</div><div class="stage-caption">Ordem textual com referências temporais explícitas em destaque.</div></div><label class="check"><input id="timed-only" type="checkbox"> Mostrar somente eventos com tempo</label></div><div class="timeline" id="timeline"></div></div>
    </section>
...
function init(){
  ...
  el('timed-only').addEventListener('change',renderTimeline);
...
function renderTimeline(){
  const onlyTimed=el('timed-only').checked,{nodes,edges}=caseData();let events=nodes.filter(n=>n.type!=='Case').sort((a,b)=>(a.sentence_id-b.sentence_id)||(a.start-b.start));
  if(onlyTimed)events=events.filter(n=>n.attributes?.time?.length||edges.some(e=>e.relation==='BEFORE'&&(e.source_id===n.node_id||e.target_id===n.node_id)));
  el('timeline').innerHTML=events.length?events.map(node=>{const times=node.attributes?.time||[];const label=times.length?times.join(' · '):`Frase ${node.sentence_id+1}`;return `<article class="event" data-node="${escapeHtml(node.node_id)}" style="--event-color:${COLORS[node.type]}"><div class="event-card"><div class="time-label">${escapeHtml(label)}</div><div><div class="event-name">${escapeHtml(node.label)}</div><div class="event-evidence">${escapeHtml(node.evidence)}</div></div><div class="type-label">${TYPE_LABEL[node.type]}</div></div></article>`}).join(''):`<div class="no-results">Nenhum evento temporal explícito neste caso.</div>`;
  el('timeline').querySelectorAll('.event').forEach(item=>item.addEventListener('click',()=>{const node=byId.get(item.dataset.node);document.querySelector('[data-panel="graph-panel"]').click();state.relationMode='all';el('graph-relation-filter').value='all';const data=aggregate(caseData().nodes,caseData().edges),group=data.nodes.find(n=>n.occurrences.some(o=>o.node_id===node.node_id));state.selected=group.id;renderGraph();showNode(group)}));
}
